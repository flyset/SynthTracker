...
Acceptance criteria
- [ ] A1) [P1] The CMake target configures and compiles all existing engine sources as C23 on macOS/Clang without C23 declaration or callback-type diagnostics.
- [ ] A2) [P1] Focused automated compile/type tests cover the observed `StartSong` declaration and `Audio.loop` callback blockers and pass under the C23 baseline.
...
- [ ] M2) Apply the smallest passing C23 fixes for the observed declaration and callback blockers.
...
- Deferred portability concern: `uint` and `machine/endian.h` require a separate Linux/GCC validation decision before source changes.
...
- Decision: The C23 baseline is validated on macOS with Clang; Linux/GCC validation is deferred.
...
- Decision: AppleClang C23 probes establish that `export` is not a C23 keyword and that the current macOS SDK supplies both `uint` transitively and `machine/endian.h`; defer cross-platform evaluation of the latter two.
- Decision: GitHub Actions is not used for Linux/GCC validation; the CI workflow was removed at user request.

Open questions
- [x] Q1) Which supported compiler/platform matrix is required for the C23 baseline?
  Resolved: macOS/Clang; Linux/GCC is deferred.
...
Decision log
- Decision (Q1): The required C23 baseline is macOS/Clang; Linux/GCC validation is deferred.
...
- [ ] S4) Refactor only as needed to clarify the macOS/Clang C23 boundary, then rerun the focused tests.
...
Validation boundary
- The compilation baseline is checked on macOS/Clang; Linux/GCC validation is deferred.
...
- Deferred portability concern: `src/tfmx.c:50-52,355` use `uint`, which is supplied transitively by the current macOS SDK but is not guaranteed by standard C23.
- Deferred portability concern: `src/player.c:5` includes `machine/endian.h`, which the current macOS SDK supplies.
