...
- Risk (uncommitted design basis): The design basis for this Track is partly uncommitted working-tree content, not historical committed evidence. `docs/ARTIFACTS.md` and `docs/adr/ADR-007-audio-frame-block-boundary-and-fixed-first-format.md` are untracked, and `README.md`, `docs/ADR.md`, `docs/ARCHITECTURE.md`, `docs/ASR.md` (including ASR-009), and `docs/GLOSSARY.md` are modified but uncommitted. These documents must be reconciled and committed according to project process before this Track can complete; otherwise the Track's normative basis could change or disappear underneath the implementation.
- Decision: Treat ADR-005, ADR-007, ASR-006, ASR-009, and `docs/ARTIFACTS.md` as the normative design basis for this Track, recording explicitly that ADR-007, ASR-009, and the artifact documentation are currently uncommitted working-tree design basis.
...
- [ ] S10) Confirm the ADR-007/ASR-009/ARTIFACTS design-basis documents have been reconciled and committed per project process, review the exact changed-file inventory and compatibility dimensions, then move Track TRACK_012 to COMPLETED with acceptance evidence.
...
- Dependency on uncommitted documentation: `docs/ARTIFACTS.md` and `docs/adr/ADR-007-audio-frame-block-boundary-and-fixed-first-format.md` are untracked working-tree files, and `README.md`, `docs/ADR.md`, `docs/ARCHITECTURE.md`, `docs/ASR.md`, and `docs/GLOSSARY.md` have uncommitted modifications. This Track's normative basis therefore currently depends on uncommitted working-tree content; S10 requires that basis to be reconciled and committed per project process before completion.
...
- [`docs/adr/ADR-007-audio-frame-block-boundary-and-fixed-first-format.md`](../../../docs/adr/ADR-007-audio-frame-block-boundary-and-fixed-first-format.md) — Audio Frame Block boundary artifact and fixed first format (currently uncommitted working-tree design basis).
- [`docs/ASR.md#asr-006--isolated-future-audio-output`](../../../docs/ASR.md) — device-independent Audio Output Port requirement.
- [`docs/ASR.md#asr-009--audio-frame-block-boundary-invariants`](../../../docs/ASR.md) — Audio Frame Block validity and distinct-failure invariants (currently uncommitted working-tree design basis).
- [`docs/ARTIFACTS.md`](../../../docs/ARTIFACTS.md) — Audio Frame Block artifact catalog, responsibilities, and deferred contract questions (currently uncommitted working-tree design basis).
...
- On completion, record: acceptance evidence per criterion, the resolved Q1–Q4 decisions, the exact changed-file inventory, the validation run output for S9, confirmation that current TFMX emission/routing and SDL-era playback behavior are observably unchanged, the reconciliation/commit status of the ADR-007/ASR-009/ARTIFACTS design basis, and whether the Phase 4 living roadmap was revised under user approval or remains current.
