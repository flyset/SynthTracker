...
void dump_macro(int *a)
{
	printf("*** dump_macro: Dumping Macro\n");
	int x=0,s=0;
...
void dump_pattern(int x)
{
	printf("*** dump_pattern: Dumping Pattern\n");
...
pattern and macro data. The routines that use the data do it for themselves.*/
	...
	// Handle
	audioData[0]=(struct Audio){
		0,                                      // pos: Position
		0x1C01,                                 // delta: Rate of change
		0x3200,                                 // slen: Length (seems to be redundant with SampleLength)
		0x15BE,                                 // SampleLength: Length of the audio sample
		&smplbuf[0x4],                    		// sbeg: Start of the audio sample buffer
		&smplbuf[0x4+0x1C42],             		// SampleStart: Start of the audio sample
		0x40,                                   // vol: Volume level
		3,                                      // mode: Playback or processing mode
		&LoopOff,                               // loop: Function pointer for loop handling
